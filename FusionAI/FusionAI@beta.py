...
def fusion_ai_online(msg):
    ...
    # March 2026 Identity Format
    fusion_identity = "You are FusionAI by Anoushka Banerjee. Stay in character. Don't use any markup symbols or languages like text or some characters to bold,underline,etc the answer you'r outputting (eg.*,_,etc)"

    # Updated Model List for March 2026 (Removed retired models)
    models_to_try = [
        'gemini-3.1-flash-lite-preview', # Newest & Most Available
        'gemini-3-flash-preview',
        'gemini-2.5-flash',              # Your "Best One"
        'gemini-2.5-flash-lite',         # Very Reliable
        'gemini-1.5-flash', 
        'gemini-1.5-flash-8b'            # The "Invincible" Fallback
    ]
    
    for i, model_name in enumerate(models_to_try):
        try:
            # TRY 1: Standard Config
            response = client.models.generate_content(
                model=model_name,
                config=types.GenerateContentConfig(
                    system_instruction=fusion_identity,
                    temperature=0.7
                ),
                contents=clean_msg
            )
            ...
            # CRITICAL: Print the first error so we can see the real cause
            if i == 0: 
                print(f"DEBUG: First model failed with: {e}")
            
            # TRY 2: Naked Fallback (Bypasses 400 Format Errors)
            try:
                # We put the identity inside the message itself
...
# ---------- GUI APPEARANCE & CHAT WINDOW ----------
def launch_ai():
    mode = selected_mode.get()
    win = Toplevel(root)
    win.title(f"FusionAI Terminal v2.6 // {mode.upper()}")
    win.geometry("750x680")
    win.configure(bg="#0B0D14") # Ultra-dark navy
    ...
    def handle_send(e=None):
        ...
        chat.config(state=NORMAL)
        chat.insert(END, "USER_INPUT: ", "user")
        chat.insert(END, f"{msg}\n")
        chat.config(state=DISABLED)
        
        status_var.set("PROCESSING_NEURAL_DATA...")
...
# App Header
Label(root, text="FUSION_AI", fg="#00D2FF", bg="#0B0D14", font=("Impact", 32)).pack(pady=(50, 5))
Label(root, text="BETA TERMINAL // SYSTEM v2.6", fg="#4B4F66", bg="#0B0D14", font=("Consolas", 10)).pack()

selected_mode = StringVar(value="offline")

def mode_container(title, val, yes_list, no_list, accent):
    """Creates a professional card-style layout for mode selection"""
    f = Frame(root, bg="#0B0D14", pady=10)
    f.pack(fill="x", padx=40)
    
    rb = Radiobutton(f, text=title, variable=selected_mode, value=val,
                     bg="#0B0D14", fg=accent, selectcolor="#0B0D14", 
                     activebackground="#0B0D14", font=("Segoe UI", 14, "bold"))
    rb.pack(anchor="w")
    
    Label(f, text=f" {yes_list}", fg="#9499A1", bg="#0B0D14", font=("Segoe UI", 8)).pack(anchor="w", padx=30)
    Label(f, text=f" {no_list}", fg="#632A2A", bg="#0B0D14", font=("Segoe UI", 8)).pack(anchor="w", padx=30)

# The Features and Disadvantages you requested
mode_container("OFFLINE_CORE", "offline", 
               "✔ INSTANT RESPONSE  ✔ ADVANCED MATHS", 
               "✖ LIMITED DATA  ✖ NO CONTEXTUAL REASONING", "#00FFC3")

mode_container("ONLINE_NEURAL", "online", 
               "✔ CLOUD-BASED INTELLIGENCE  ✔ FULL EXPLANATIONS", 
               "✖ REQUIRES INTERNET  ✖ API RATE LIMITS", "#3D99FF")
...
btn_launch.pack(pady=60)

# Footer Info
Label(root, text="DESIGNED BY ANOUSHKA BANERJEE", fg="#2D3247", bg="#0B0D14", font=("Consolas", 8)).pack(side="bottom", pady=15)
