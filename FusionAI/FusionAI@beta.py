...
def launch_ai():
    ...
    def typewriter(text):
        ...
        # Start speaking AS SOON AS typing starts for a natural feel
        if voice_on.get():
        ...
        def add_char(i=0):
            if i < len(text):
                ...
                chat.config(state=DISABLED)
                chat.see(END)
                win.after(10, lambda: add_char(i + 1))
            else:
                chat.config(state=NORMAL)
                chat.insert(END, "\n\n")
                chat.config(state=DISABLED)
...
mode_container("OFFLINE_CORE", "offline", "✔ INSTANT RESPONSE", "✖ LIMITED DATA", "#00FFC3")
mode_container("ONLINE_NEURAL", "online", "✔ CLOUD INTELLIGENCE", "✖ API RATE LIMITS", "#3D99FF")
